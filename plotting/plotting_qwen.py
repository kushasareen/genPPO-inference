...
json_files = ["20250204-152716_rebase_math", "20250204-170948_rebase_math", "20250204-200608_bestofn_math"]
...
method_names = ["Rebase (Qwen_0.3)", "Rebase (Qwen_0.3_adv)", "Sampling (Qwen_0.3)"]
...
# }
base_colors = {
    "Rebase (Qwen_0.3)": "#1f77b4",  # Blue
    "Rebase (Qwen_0.3_adv)": "#ff7f0e",  # Orange
    "Sampling (Qwen_0.3)": "#2ca02c",  # Green
    "Sampling (Qwen Base)": "#d62728",  # Red
...
def get_linestyles(num_linestyles):
    ...
    return [linestyles[i] for i in range(num_linestyles)]
    ...
    for method, content in data.items():
        ...
        if subcategories:
            ...
            for subcat in subcats:
                ...
                linestyle = get_linestyles(len(subcats))[subcat_idx]
                if method_name == "Sampling (VinePPO)" or method_name == "Sampling (Qwen Base)":
                    subcat_name = "solution"

                plt.plot(x_values, y_values, label=f"{method_name} ({subcat_name})", color=color, linestyle=linestyle)
        else:
...
best_subcategory_dict = {'Rebase (Qwen_0.3)': 'sum', 'Rebase (Qwen_0.3_adv)': 'min', 'Sampling (Qwen_0.3)': 'min', 'Sampling (Qwen Base)': 'last'}
best_category_dict = {'Rebase (Qwen_0.3)': 'weighted_majority_vote', 'Rebase (Qwen_0.3_adv)': 'weighted_majority_vote', 'Sampling (Qwen_0.3)': 'weighted_majority_vote', 'Sampling (Qwen Base)': 'best_of_n'}
