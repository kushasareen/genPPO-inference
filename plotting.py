...
json_files = ["20241129-232331_rebase", "20241129-230427_bestofn_vineppo", "20241129-081543_bestofn_ckpt2", "20241205-233059_beamsearch_ckpt2"]  # Replace with actual file names
method_names = ["Rebase (GenPPO)", "Sampling (VinePPO)", "Sampling (GenPPO)", "Beam Search (GenPPO)"]  # Replace with actual method names
...
def get_shades(base_color, num_shades):
    ...
    return [
        (rgba[0] + 0.9 * (1 - rgba[0]) * (i / (num_shades + 1)),
         rgba[1] + 0.9 * (1 - rgba[1]) * (i / (num_shades + 1)),
         rgba[2] + 0.9 * (1 - rgba[2]) * (i / (num_shades + 1)),
         1.0)
...
def plot_category(data, category, subcategories=False, title=None, xlabel="N", ylabel="Accuracy", yscale="linear"):
    """
...
# Majority Vote
plot_category(data, "majority_vote", title="Majority Vote")
...
best_subcategory_dict = {'Rebase (GenPPO)': 'min', 'Sampling (VinePPO)': 'sum', 'Sampling (GenPPO)': 'last', 'Beam Search (GenPPO)': 'min'}
best_category_dict = {'Rebase (GenPPO)': 'weighted_majority_vote', 'Sampling (VinePPO)': 'majority_vote', 'Sampling (GenPPO)': 'weighted_majority_vote', 'Beam Search (GenPPO)': 'best_of_n'}
