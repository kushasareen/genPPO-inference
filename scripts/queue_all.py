...
parser.add_argument('--model', type=str, default='qwen_genPPO_0.3')
...
command = f'bash scripts/run.sh -m {model_path} -d {dataset} -p {dataset_path} -k {K_max} -s {search_alg} -o {output_dir} -e 0'
print(command)
os.system(command)
num_jobs += 1
...
while k < K_max:
    for seed in range(num_seeds):
        command = f'bash scripts/run.sh -m {model_path} -d {dataset} -p {dataset_path} -k {k} -s {search_alg} -o {output_dir} -e {seed}'
        print(command)
        os.system(command)
        num_jobs += 1
        # QUEUE 
