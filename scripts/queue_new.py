...
for model in ['sft_no_instr_1', 'clf_no_instr_1']:
# for model in ['grpo_base_1_42', 'grpo_base_0_42']:
# for model in ['qwen_genPPO_10', 'qwen_ppo']:
    ...
    for dataset in ['math128']:
        ...
        script = 'run_main_vppo.sh'
        # if model == 'qwen_ppo':
        #     script = 'run_ppo.sh'
        #     search_alg = 'bestofn_qwen_ppo'

        command = f'bash scripts/{script} -m {model_path} -d {dataset} -p {dataset_path} -k {K_max} -s {search_alg} -o {output_dir} -n {num_samples} -t {max_tokens} -e 0'
        print(command)
